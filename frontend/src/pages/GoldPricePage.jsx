...
const CalendarView = () => {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        ...
        setHistoricalEvent({
          event: "Hãy bắt đầu ngày mới với niềm vui và hy vọng!",
        }); // fallback
      } finally {
  ...
  const renderDateDetails = () => {
    ...
    if (shenType === "黃道" || shenType === "黄道")
      dayTianShen = "Ngày Hoàng Đạo";
    ...
    return (
      <div className="bg-linear-to-br from-amber-500 to-orange-600 rounded-3xl p-6 text-white text-center shadow-lg shadow-orange-500/30 flex flex-col justify-center relative overflow-hidden h-full min-h-[420px]">
        ...
        <h2 className="text-xl font-medium opacity-90 mb-1 z-10">
          {DAYS_OF_WEEK[selectedDate.getDay()]},{" "}
          {format(selectedDate, "dd/MM/yyyy")}
        </h2>
        <div className="text-7xl font-bold my-4 z-10">
          {format(selectedDate, "dd")}
        </div>

        <p
          className={`text-sm font-bold mt-2 z-10 py-1.5 px-4 rounded-full inline-block mx-auto backdrop-blur-sm self-center transition-all duration-300 ${
            isTdy
              ? "bg-white/30 text-white"
              : "bg-white/90 text-orange-600 shadow-md transform scale-105"
          }`}
        >
          ...
          {loadingEvent ? (
            ...
            <div className="flex flex-col gap-1 items-center w-full">
              <p className="text-[10px] font-bold text-yellow-200/80 uppercase tracking-widest self-start">
                {historicalEvent?.type === "VietnamWar"
                  ? "🇻🇳 Sự kiện lịch sử"
                  : historicalEvent?.type === "UncleHo"
                    ? "🇻🇳 Dấu ấn Bác Hồ"
                    : "💡 Suy ngẫm"}
              </p>
              <span className="text-left w-full text-[13px] leading-relaxed font-medium">
        ...
        <div className="mt-auto border-t border-white/20 pt-4 space-y-3 z-10 text-left">
          <div className="flex justify-between items-center mb-1 gap-2">
            <p className="font-semibold text-2xl shrink-0">
              Âm: {textInfo.day}/{textInfo.month}
            </p>
            <span
              className={`text-[10px] sm:text-xs px-2 py-1 rounded font-bold uppercase tracking-wide text-center leading-tight ${
                dayTianShen === "Ngày Hoàng Đạo"
                  ? "bg-amber-200 text-amber-800"
                  : dayTianShen === "Ngày Hắc Đạo"
          ...
          <div className="grid grid-cols-2 gap-2 text-[13px] opacity-95">
            <div className="bg-black/10 rounded-xl p-3">
              <p className="text-white/70 text-[10px] mb-1 font-bold uppercase tracking-wider">
                Can Chi
              </p>
              <div className="space-y-0.5">
                <p className="whitespace-nowrap overflow-hidden text-ellipsis">
                  ...
                  <span className="font-semibold">
                    {textInfo.yearText.replace("Năm ", "")}
                  </span>
                </p>
                <p className="whitespace-nowrap overflow-hidden text-ellipsis">
                  - Tháng{" "}
                  <span className="font-semibold">
                    {textInfo.monthText.replace("Tháng ", "")}
                  </span>
                </p>
                <p className="whitespace-nowrap overflow-hidden text-ellipsis">
                  - Ngày{" "}
                  <span className="font-semibold">
                    {textInfo.dayText.replace("Ngày ", "")}
                  </span>
                </p>
              </div>
            </div>
            <div className="bg-black/10 rounded-xl p-3 flex flex-col h-full">
              <p className="text-yellow-200 text-[10px] mb-1 font-bold uppercase tracking-wider">
                Giờ Hoàng Đạo
              </p>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in zoom-in-95 duration-500">
      <div className="lg:col-span-1">{renderDateDetails()}</div>
      <div className="lg:col-span-2 bg-white/40 backdrop-blur-xl border border-white/60 p-6 rounded-3xl shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <button
            onClick={handlePrevMonth}
            className="p-2 bg-white/60 hover:bg-white rounded-full transition-colors text-gray-600 shadow-sm border border-white"
          >
            <ChevronLeft size={20} />
          </button>
        ...
        <div className="grid grid-cols-7 gap-1 md:gap-2">
          {blanks.map((_, i) => (
            <div key={`blank-${i}`} className="p-2 h-16 md:h-24" />
          ))}
          {days.map((date, i) => {
            ...
            return (
              <div
                ...
                className={`relative flex flex-col items-center justify-center p-1 md:p-2 rounded-xl h-16 md:h-24 transition-all cursor-pointer border
                  ${
                    isSel
                      ? "bg-orange-500 border-orange-500 text-white shadow-md transform scale-105 z-10"
                      : isTdy
              ...
              >
                <span
                  className={`text-lg md:text-2xl font-bold ${isSel ? "text-white" : isTdy ? "text-orange-600" : ""}`}
                >
                  {format(date, "d")}
                </span>
                <span
                  className={`text-[11px] md:text-sm font-medium ${isSel ? "text-orange-100" : isTdy ? "text-orange-500" : "text-gray-400"}`}
                >
          ...
          <div>
            ...
            <p className="text-gray-600 flex items-center gap-2">
              {activeTab === "gold"
                ...
                : "Xem lịch âm và lịch dương chi tiết trong tháng"}
            </p>
          </div>
            ...
            {loading ? (
              <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4">
                {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
                  <div
                    key={i}
                    className="animate-pulse bg-white/60 h-[140px] rounded-2xl border border-white shadow-sm flex flex-col justify-between p-4"
                  >
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4">
                {Object.values(data?.prices || {}).map((item, idx) => {
                  const spread =
                    item.sell && item.buy ? item.sell - item.buy : 0;
                  return (
                    <div
                      key={idx}
                    ...
                    >
                      <div className="flex justify-between items-start mb-2">
                        <h3
                          className="font-bold text-[13px] md:text-sm text-gray-800 line-clamp-1 pr-1"
                      ...
                      <div className="flex-1 flex flex-col gap-1.5">
                        <div className="flex justify-between items-center p-2 rounded-xl bg-gray-50/50 group-hover:bg-gray-50 transition-colors border border-transparent group-hover:border-gray-100">
                          ...
                          <div className="text-right flex items-center gap-1.5">
                            ...
                            {item.change_buy !== 0 && (
                              <div
                                className={`text-[9px] flex items-center gap-0.5 px-0.5 rounded font-medium ${getChangeClass(item.change_buy)}`}
                                title={formatPrice(Math.abs(item.change_buy))}
                              >
                        ...
                        <div className="flex justify-between items-center p-2 rounded-xl bg-gray-50/50 group-hover:bg-gray-50 transition-colors border border-transparent group-hover:border-gray-100">
                          ...
                          <div className="text-right flex items-center gap-1.5">
                            ...
                            {item.change_sell !== 0 && (
                              <div
                                className={`text-[9px] flex items-center gap-0.5 px-0.5 rounded font-medium ${getChangeClass(item.change_sell)}`}
                                title={formatPrice(Math.abs(item.change_sell))}
                              >
