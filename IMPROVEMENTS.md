...
- [ ] **Quản lý Context Window / Lịch sử hội thoại:** Xử lý việc không gửi toàn bộ dữ liệu lịch sử lớn từ frontend (có thể Backend sẽ tự quản lý session history, hoặc dùng Sliding Window).
...
- [ ] **Logic phát hiện Table an toàn hơn:** Nâng cấp cách phân tích Markdown thành Excel, khắc phục điểm yếu của hàm `hasTable` hiện tại.
